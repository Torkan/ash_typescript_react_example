defmodule AshTypescriptReactExample.Invoicing.Invoice do
  ...
  attributes do
    ...
    # Invoice metadata
    attribute :serial_number, :string, allow_nil?: true, public?: true # Only set when finalized
  ...
  actions do
    ...
    create :create do
      primary? true
      accept [:issue_date, :due_date, :currency,
              :company_name, :company_address_line_1, :company_address_line_2, :company_city, 
              :company_postal_code, :company_country, :company_vat_number, :company_email, :company_phone,
              :customer_name, :customer_address_line_1, :customer_address_line_2, :customer_city,
              :customer_postal_code, :customer_country, :customer_vat_number, :customer_email, :customer_phone]
      ...
      change fn changeset, _context ->
        ...
        case sequence_result do
          {:ok, sequence} ->
            ...
            serial_number = "INV-#{current_year}-#{String.pad_leading(to_string(sequence.next_number), 4, "0")}"
          ...
          {:error, error} ->
            Ash.Changeset.add_error(changeset, "Failed to assign serial number: #{inspect(error)}")
        end
      end
    end
