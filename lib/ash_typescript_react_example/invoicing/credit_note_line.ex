defmodule AshTypescriptReactExample.Invoicing.CreditNoteLine do
  @moduledoc """
  CreditNoteLine resource representing individual line items for credit notes.

  Similar structure to InvoiceLine but for credit note documents.
  ...
  Line items can only be modified when the parent credit note is in draft state.
  """
  ...
  postgres do
    ...
    custom_indexes do
      ...
      index [:credit_note_id, :line_number], unique: true
    end
  end

  # Clean TypeScript type name
  typescript do
    type_name "CreditNoteLine"
  end
  ...
  actions do
    ...
    create :create do
      primary? true
      accept [:credit_note_id, :line_number, :description, :quantity, :unit_price, :tax_rate]
    end
    ...
    read :list_for_credit_note do
      # Get all lines for a specific credit note, ordered by line_number
      argument :credit_note_id, :uuid, allow_nil?: false
      filter expr(credit_note_id == ^arg(:credit_note_id))
      prepare build(sort: [:line_number])
    end

    read :get_by_credit_note_and_line_number do
      # Get a specific line by credit note and line number
      argument :credit_note_id, :uuid, allow_nil?: false
  ...
  policies do
    # Lines inherit access control from their parent credit note
    ...
    policy action_type([:create, :update, :destroy]) do
      authorize_if actor_present()
      # Additional validation: only allow modifications when parent credit note is draft
  ...
  attributes do
    ...
    attribute :credit_note_id, :uuid, allow_nil?: false, public?: true
  ...
  relationships do
    belongs_to :credit_note, AshTypescriptReactExample.Invoicing.CreditNote do
      allow_nil? false
  ...
  identities do
    # Ensure unique line numbers per credit note
    identity :unique_credit_note_line_number, [:credit_note_id, :line_number]
  end
end
