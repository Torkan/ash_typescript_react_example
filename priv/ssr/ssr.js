...
var Router = class {
  ...
  visit(href, options = {}) {
    ...
    if (prefetched) {
      ...
      reveal(true);
      requestStream.send(Request$1.create(requestParams, page.get()));
    }
  }
  ...
  prefetch(href, options = {}, prefetchOptions = {}) {
    ...
    ensureCurrentPageIsSet().then(() => {
      prefetchedRequests.add(
        requestParams,
        (params) => {
          this.asyncRequestStream.send(Request$1.create(params, page.get()));
        },
        {
          cacheFor: 3e4,
          cacheTags: [],
...
var htmlEscape;
var hasRequiredHtmlEscape;

function requireHtmlEscape () {
	if (hasRequiredHtmlEscape) return htmlEscape;
	hasRequiredHtmlEscape = 1;
...
var htmlEscapeExports = requireHtmlEscape();
const escape$1 = /*@__PURE__*/getDefaultExportFromCjs(htmlEscapeExports);

Render[FILENAME] = 'node_modules/@inertiajs/svelte/dist/components/Render.svelte';
...
const h = (component, propsOrChildren, childrenOrKey, key = null) => {
	...
	return {
		...
		key: hasProps
			? key
			: typeof childrenOrKey === "number" ? childrenOrKey : null,
		...
		children: hasProps
			? Array.isArray(childrenOrKey)
				? childrenOrKey
				: childrenOrKey !== null ? [childrenOrKey] : []
			: Array.isArray(propsOrChildren)
				? propsOrChildren
				: propsOrChildren !== null ? [propsOrChildren] : []
	};
};
...

					{
						children: prevent_snippet_stringification(($$payload) => {
							...
							for (let $$index = 0, $$length = each_array.length; $$index < $$length; $$index++) {
								...
								Render($$payload, spread_props([child]));
								$$payload.out.push(`<!---->`);
							}
...
App[FILENAME] = 'node_modules/@inertiajs/svelte/dist/components/App.svelte';
...
function App($$payload, $$props) {
	...
	function resolveRenderProps(component2, page2, key2 = null) {
		...
		return layout
			? resolveLayout(layout, child, page2.props, key2)
			: child;
	}
...
function render(page) {
  return createInertiaApp({
    page,
    resolve: async (name) => await __variableDynamicImportRuntimeHelper((/* #__PURE__ */ Object.assign({})), `./pages/${name}.svelte`, 3),
    setup({ el, App }) {
      mount();
    },
  });
